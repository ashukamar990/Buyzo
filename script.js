...
// Firebase imports (will be loaded from the module in HTML)
let database;
try {
...
function saveOrdersToFirebase(orders) {
  if (!database) {
    // Fallback to localStorage if Firebase is not available
  ...
  set(ref(database, 'orders'), orders)
    .then(() => {
      console.log('Orders saved to Firebase:', orders);
    })
    .catch((error) => {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Load orders from Firebase on page load
  getOrdersFromFirebase((orders) => {
